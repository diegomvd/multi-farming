...
case class Landscape(radius : Int,
                     connections : ParMap[ModuloCoord,Vector[ModuloCoord]],
                     composition : ParMap[ModuloCoord,EcologicalUnit]){

  // method to find a land unit given its position
  def findUnit(pos: ModuloCoord) : EcologicalUnit = map(getID(location, size)) // doesn't seem needed

  // method to update the landscape, unsure about the syntax...
  def update(pos: ModuloCoord, unit: EcologicalUnit): Landscape = new Landscape(this.radius, this.connections, this.composition.map{ case (pos,_) => pos -> unit })
...
object Landscape {
  ...
  def buildConnections(radius: Int, threshold: Int): ParMap[ModuloCoord, Vector[ModuloCoord]] =
    ModuloCoord.apply(radius).map( pos => pos -> pos.manhattanNeighbors(radius,threshold).toVector ).toMap.par

  // Builds a pristine composition that is later updated in function of input parameters
  def buildPristineComposition(radius: Int): ParMap[ModuloCoord, EcologicalUnit] =
    ModuloCoord.apply(radius).map( pos => pos -> EcologicalUnit(pos,"Natural",1.0) ).toMap.par
  ...
  def buildNaturalFragments(pristine_composition: ParMap[ModuloCoord, EcologicalUnit]): ParSet[NaturalFragment] =
  ...
  def buildStrategicUnits(n_units: Int, radius: Int, sparingFraction: Double) = ParMap[ModuloCoord, String] {
