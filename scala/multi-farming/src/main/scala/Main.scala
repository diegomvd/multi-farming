  ...
  def parseParameters(params: Seq[String]): Parameters =
    if (params.size>0)
      params.flatMap(_.split('=')).sliding(2,2).foldLeft(Parameters()){
        case (acc, Seq("size", value))                         => if(value>=1) acc.copy(size = Integer.parseInt(value))             else println(s"parseParameters.error: trying to set size to $value but size must be equal to or larger than 1.")
